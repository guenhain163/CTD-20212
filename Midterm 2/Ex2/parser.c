...
void compileAssignSt(void) {
  ...
  lvarType = (Type**)malloc(sizeof(Type));

  int counterForVariables = 1;
  lvarType = (Type**)realloc(lvarType, counterForVariables * sizeof(Type));
  lvarType[0] = compileLValue();
  while (lookAhead->tokenType == SB_COMMA) {
    ...
    counterForVariables++;
    lvarType = (Type**)realloc(lvarType, counterForVariables * sizeof(Type));
    lvarType[counterForVariables - 1] = compileLValue();
  }
  
  eat(SB_ASSIGN);
  int counterForExpression = 1;
  expType = compileExpression();
  checkTypeEquality(lvarType[0], expType);
  while (lookAhead->tokenType == SB_COMMA) {
    eat(SB_COMMA);
    counterForExpression++;
    expType = compileExpression();
    if (lvarType[counterForExpression - 1] != NULL) {
      checkTypeEquality(lvarType[counterForExpression - 1], expType);
    }
  }
